$(function () {
    ...
    $('.near2').each(function () {
        let ths = $(this);
        let selectParent = ths.find('.near__select-wrapper');
        ths.find('.near__tab2').click(function () {
            ths.find('.near__tab2').removeClass('near__tab-active').eq($(this).index()).addClass('near__tab-active');
            ths.find('.near__tab-item').hide().eq($(this).index()).fadeIn();
            newsSlider1.slick('refresh');
        }).eq(0).addClass('near__tab-active');

        ths.find('.js-near-select').click(function () {
            $(this).toggleClass("is-active");
            selectParent.find(".near__select-items").toggle();;
        });
    });

    const mobileTabs = Array.from(document.querySelectorAll(".js-near-tab"));

    mobileTabs.forEach(tab => {
        tab.addEventListener("click", ()=> {
            const parent = tab.closest(".near__select-wrapper");
            const text = tab.querySelector(".js-tab-text").innerHTML;

            parent.querySelector(".js-parent-slider").innerHTML = text;
            parent.querySelector(".near__select-items").style.display = "none";
        });
    });
